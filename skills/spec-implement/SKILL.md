...
2. **Work the tasks in the plan's order.** For each task:
...
- Behavioural risk lives where the delta says it does — a task that changes
  failure behaviour or defaults deserves its tests written first.
